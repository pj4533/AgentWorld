...
class ServerConnectionManager {
    ...
    func updateWorld(_ newWorld: World) {
        ...
        // Use try-catch to handle any potential exceptions in logging
        do {
            // Get counts safely
            ...
            logger.error("Error during world update logging: \(error.localizedDescription)")
            // Still update the world even if logging fails
            self.world = newWorld
        }
    }
    ...
    func sendObservationsToAll(timeStep: Int) {
        ...
        // Use try-catch to prevent any logging exceptions from crashing
        do {
            ...
            for agentId in currentConnections.keys {
                ...
                if let observation = world.createObservation(for: agentId, timeStep: timeStep) {
                    ...
                    // Log detailed information about what we're sending (simplified format)
            ...
            logger.error("Error during sendObservationsToAll: \(error.localizedDescription)")
        }
    }
